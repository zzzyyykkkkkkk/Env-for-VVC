...
class PowerNetEnv_IEEE123(gym.Env):
    ...
    def __init__(self, env_config: dict = env_config) -> None:
        ...
        self.epsilon = 0.02  # 因为根据灵敏度矩阵来保证确定电压在安全范围虽然已经是一种比较稳妥的做法，但还是会出现调整之后无法达到安全范围，所以给出一定裕度来保证
    ...
    def _find_safe_action(self, action):
        ...
        if all(self.v_min <= v <= self.v_max for v in V_A):
            return action  # 如果所有电压都在安全范围内，直接返回原始动作

        # 步骤2：计算电压灵敏度矩阵
        J = net_copy._ppc['internal']['J'] # 尺寸为 2(N -1)x2(N - 1)，其中 N 是总节点数，去除了参考母线(Slack bus)
        J_dense = J.toarray()
        n = net_copy.bus.shape[0] - 1
        B = J_dense[n:, n:]
        try:
            S_vq = np.linalg.inv(B)
        except np.linalg.LinAlgError:
            S_vq = np.linalg.pinv(B)  # 如果不可逆，使用伪逆

        # 步骤3：定义可控设备
        pv_buses = self.network_info['pv_buses']
        svc_buses = self.network_info['svc_buses']
        C = pv_buses + svc_buses
        C_idx = [net_copy.bus.index.get_loc(bus) for bus in C]
        m = len(C)

        # 获取当前无功功率（原始动作对应的）
        pv_sgen_idx = [net_copy.sgen.index[net_copy.sgen.bus == bus].tolist()[0] for bus in pv_buses]
        svc_shunt_idx = [net_copy.shunt.index[net_copy.shunt.bus == bus].tolist()[0] for bus in svc_buses]
        Q_A_pv = [net_copy.sgen.at[idx, 'q_mvar'] for idx in pv_sgen_idx]
        Q_A_svc = [net_copy.shunt.at[idx, 'q_mvar'] for idx in svc_shunt_idx]
        Q_A_C = Q_A_pv + Q_A_svc

        # 获取无功功率的最大值
        Q_max_pv = self.network_info['pv_q_mvar']
        Q_max_svc = self.network_info['q_mvar']
        Q_max_C = list(Q_max_pv) + list(Q_max_svc)

        # 步骤4：识别电压超限的节点
        D_min = [i for i, v in enumerate(V_A) if v < self.v_min]
        D_max = [i for i, v in enumerate(V_A) if v > self.v_max]

        # 步骤5：设置优化问题
        x = cp.Variable(m)  # Delta Q_C
        objective = cp.Minimize(cp.sum_squares(x))
        constraints = []

        # 无功功率调整的边界
        for j in range(m):
            lb = -Q_max_C[j] - Q_A_C[j]
            ub = Q_max_C[j] - Q_A_C[j]
            constraints.append(x[j] >= lb)
            constraints.append(x[j] <= ub)

        # 电压约束
        for k in D_min:
            coeffs = [S_vq[k-1, C_idx[j]] for j in range(m)]
            constraints.append(sum(coeffs[j] * x[j] for j in range(m)) >= self.v_min - V_A[k] + self.epsilon)
        for k in D_max:
            coeffs = [S_vq[k-1, C_idx[j]] for j in range(m)]
            constraints.append(sum(coeffs[j] * x[j] for j in range(m)) <= self.v_max - V_A[k] - self.epsilon)
        ...
        if prob.status == 'optimal':
            Delta_Q = x.value
            Q_safe_C = [Q_A_C[j] + Delta_Q[j] for j in range(m)]
            # 将调整后的无功功率转换回归一化动作
            A_pv = [Q_safe_C[j] / Q_max_pv[j] for j in range(len(pv_buses))]
            A_svc = [Q_safe_C[j + len(pv_buses)] / Q_max_svc[j] for j in range(len(svc_buses))]
            A_bat = action[len(pv_buses) + len(svc_buses):]  # 电池动作不变
            action_new = np.concatenate([A_pv, A_svc, A_bat])
            net_copy_new = copy.deepcopy(self.net)
            self._apply_action_to_net(net_copy_new, action_new)
            pp.runpp(net_copy_new, calculate_voltage_angles=True, trafo_model="t", numba=False)
            V_A_new = net_copy_new.res_bus.vm_pu.values
            if all(self.v_min <= v <= self.v_max for v in V_A_new):
                print(f"安全约束后最大电压差值 {np.max(np.abs(V_A_new - 1.0)):.4f}")
                return action_new
            else:
                print(f"安全约束后最大电压差值 {np.max(np.abs(V_A_new - 1.0)):.4f}")
                print('无论智能体如何动作都没办法将电压限制在安全运行范围内')
                return action_new
        else:
            # 如果优化失败，使用零动作
    ...
    def step(self, action):
        ...
        action_real, voltages = self._apply_actions(safe_action)
        reward = self._calculate_reward(voltages, action_real)
        self.current_time += 1
        ...
        dv_max = self._get_max_voltage_deviation()  # ← 新增
        soc_mean, soc_min, soc_max = self._get_soc_stats()  # ← 新增
        info = {
            'line_loss': self.net.res_line.pl_mw.sum(),
            'max_voltage_deviation': dv_max,
            "soc_mean": soc_mean,
            "soc_min": soc_min,
            "soc_max": soc_max
        }
    ...
    #     action_real, vm_pu = self._apply_actions(action)
    #     reward = self._calculate_reward(vm_pu, action_real)  # ← 传入智能体实际动作的值
    #
    #     self.current_time += 1
    #     terminated = self.current_time >= self.episode_length
    ...
    #
    #
    #     info = {"loss_mw": self.net.res_line.pl_mw.sum(),
    #             "dv_max": dv_max,
    #             "soc_mean": soc_mean,
    #             "soc_min": soc_min,
    #             "soc_max": soc_max
    ...
    def _calculate_reward(self, vm_pu: np.ndarray, action_real: np.ndarray) -> float:
        if self.reward_fun_type == 'Bowl':
            ...
            line_loss = self.net.res_line.pl_mw.sum()
            # 奖励：负的电压屏障值和动作惩罚值
            reward = -1 * voltage_penalty - 0.01 * action_penalty - 0.01 * line_loss  # 权重可根据需要调整
